...
private:
  ...
  template <typename T> void Substitution(T *out, const T *in) {
    const T *pt = in;
    T *pto = out;
    T a, b, c;
  ...
  void init(bool *key_b) {
    ...
    // rounds*blocksize*blocksize);
    len = rounds * blocksize * blocksize;
    // LinMatrices = new Bit[len];
    LinMatrices_loc = new bool[len];
    prg.random_bool(LinMatrices_loc, len);
    // ProtocolExecution::prot_exec->feed((block*)LinMatrices, PUBLIC,
    // LinMatrices_loc, len);

    // roundconstants: r * n
    // random_bit_vec_gen(roundconstants, roundconstants_loc, rounds *
    // blocksize);
    ...
    // keyMatrices: r * n * k
    // random_bit_vec_gen(KeyMatrices, KeyMatrices_loc, rounds * blocksize *
    // keysize);
    len = (rounds + 1) * blocksize * keysize;
    // KeyMatrices = new Bit[len];
    KeyMatrices_loc = new bool[len];
    prg.random_bool(KeyMatrices_loc, len);
    // ProtocolExecution::prot_exec->feed((block*)KeyMatrices, PUBLIC,
    // KeyMatrices_loc, len);
